...
import {month} from '../../api/school'
import {week} from '../../api/school'
import {day} from '../../api/school'
export default {
  data() {
    return {
      option:'',
   ...
   getArrEqual(arr1, arr2) {
      ...
      for (let i = 0; i < arr1.length; i++) {
        for (let j = 0; j < arr2.length; j++) {
          ...
          if (i > 0) {
            if (arr1[i] == arr2[j]) {
              newArr[j] = this.weeklist1[i].count ;
        ...
        .then((res) => {
          ...
          for (let i in res.data.data) {
            if (res.data.data[i].type == "JYHP01") {
              ...
              this.num2.push(res.data.data[i].count );
            }
          }
    ...
    getChart(a, b,c) {
      ...
      myChart6.on("click", function (params) {
        var dataweek = [];
        var dataweek1 = [];
        week(that.datearr[params.dataIndex] + "-01", "吉叶大学").then(
          (res) => {
            console.log(res);
            ...
            for (let i in res.data.data) {
              if (res.data.data[i].type == "JYHP01") {
                ...
                dataweek.push(res.data.data[i].count );
              } else {
                dataweek1.push(res.data.data[i].count );
              }
            }
            ...
            myChart6.on("click", function (params) {
              ...
              day(this.weekday[params.dataIndex].week, "吉叶大学").then(
                (res) => {
                  console.log(res);
                  for (let i in res.data.data) {
                    if (res.data.data[i].type == "JYHP01") {
                      dataday.push(res.data.data[i].count );
                    } else {
                      dataday1.push(res.data.data[i].count );
                    }
                  }
                  this.option = {
                    tooltip: {
